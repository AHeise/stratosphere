 ...
 * "Hash joins and hash teams in Microsoft SQL Server", by Goetz Graefe et al. In its current state, the implementation
 ...
 * 
 * @param <BT>
 *        The type of records from the build side that are stored in the hash table.
 * @param <PT>
...
public class MutableHashTable<BT, PT> implements MemorySegmentSource {
	...
	/**
	 * The size of the segments used by the hash join buckets. All segments must be of equal size to ease offset
	 * computations.
	 */
	 ...
	 * @throws IOException
	 *         Thrown, if the thread is interrupted while grabbing the next buffer. The I/O
	 ...
	 * available.
	 * <p>
	 * The current logic makes sure that there are always between 10 and 127 partitions, and close to 0.1 of the number
	 * of buffers.
	 * 
	 * @param numBuffers
			...
			while (true) {
				...
				while (this.numInSegment < this.countInSegment) {
					...
					if (thisCode == this.searchHashCode) {
						...
						// deserialize the key to check whether it is really equal, or whether we had only a hash
						// collision
						try {
